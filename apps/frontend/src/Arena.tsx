...
const Arena = () => {
  ...
  const [users, setUsers] = useState<any>(new Map());
  const [params, setParams] = useState<any>({ token: "", mapId: "" });

  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const token = urlParams.get("token") || "";
    const mapId = urlParams.get("mapId") || "";
    setParams({ token, mapId });
  ...
  const handleWebSocketMessage = (message: any) => {
    switch (message.type) {
      ...
      case "movement":
        setUsers((prev: any) => {
          const newUsers = new Map(prev);
          const user = newUsers.get(message.payload.userId) as {
            x: number;
            y: number;
          };
          if (user) {
  ...
  return (
    <div className="p-4" onKeyDown={handleKeyDown} tabIndex={0}>
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600">Map ID: {params.mapId}</p>
